...
- **data_file** (string, required): Path to the data file to analyze (supports CSV, JSON, Excel, Parquet)
- **analysis_type** (string): Type of analysis - options are 'quick', 'comprehensive', 'statistical', 'exploratory'
- **output_dir** (string): Directory where analysis results and visualizations will be saved
- **include_visualizations** (string): Whether to generate visualizations (true/false)
- **report_format** (string): Output report format - options are 'markdown', 'html', 'pdf'


## Example Usage

```
/data-analysis-pipeline `data_file=value`, `analysis_type=comprehensive`, `output_dir=./analysis_output`, `include_visualizations=true`, `report_format=markdown`
```
