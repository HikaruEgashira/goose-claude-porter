...
Automatically generates comprehensive documentation for codebases by analyzing source files, extracting APIs, and creating formatted documentation with examples and best practices
...
- **source_path** (string): Path to the source code directory or specific file to document
- **doc_format** (string): Documentation format: 'markdown', 'html', 'restructuredtext', 'javadoc', 'jsdoc'
- **include_examples** (string): Whether to generate usage examples for documented items (true/false)
- **output_location** (string): Directory where documentation files will be saved
- **doc_style** (string): Documentation style: 'minimal' (brief descriptions), 'standard' (descriptions + params), 'comprehensive' (full with examples and cross-refs)
- **language_hint** (string): Optional: specify programming language if auto-detection is insufficient (e.g., 'python', 'javascript', 'rust', 'go')


## Example Usage

```
/code-documentation-generator `source_path=.`, `doc_format=html`, `include_examples=true`, `output_location=./docs`, `doc_style=comprehensive`, `language_hint=value`
```
