...
- **data_file** (string, required)
  Path to the data file to clean

- **handle_missing** (string)
  Strategy for missing values - options are 'remove', 'mean', 'median', 'mode', 'smart'

- **remove_duplicates** (string)
  Whether to remove duplicate rows (true/false)

- **outlier_strategy** (string)
  How to handle outliers - options are 'keep', 'remove', 'flag', 'cap'
