...
An intelligent code review assistant that learns your preferences and provides personalized, actionable feedback on code changes with improvement suggestions
...
- **review_scope** (string): Scope of changes to review: 'staged' (staged changes), 'unstaged' (working directory), 'commit' (last commit), 'branch' (all commits in current branch vs main)
- **review_depth** (string): Review depth level: 'quick' (focus on critical issues), 'balanced' (standard review), 'thorough' (detailed analysis including documentation and tests)
- **focus_areas** (string): Comma-separated focus areas: 'security', 'performance', 'readability', 'testing', 'documentation', 'architecture', or 'all'
- **language_specific** (string): Optional: specify programming language for language-specific best practices (e.g., 'python', 'javascript', 'rust')
- **project_context** (string): Optional: brief project context or specific concerns to prioritize in this review


## Example Usage

```
/code-review-mentor `review_scope=staged`, `review_depth=balanced`, `focus_areas=all`, `language_specific=value`, `project_context=value`
```
