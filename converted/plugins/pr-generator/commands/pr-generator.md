...
Automatically generate pull request descriptions based on changes in a local git repo.
...
- **git_repo_path** (string, required): Path to the local git repository
- **push_pr** (boolean): Whether to push changes and create a PR


## Example Usage

```
/pr-generator `git_repo_path=value`, `push_pr=value`
```
