...
```bash
/plugin install migrate-cypress-tests-to-playwright
```
...
- **cypress_test_file** (file, required)
  The specific Cypress test file to migrate (e.g., cypress/e2e/login.cy.js)

- **target_directory** (file, required)
  The target directory where the Playwright test should be created
