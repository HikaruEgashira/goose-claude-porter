---
name: migrate-cypress-tests-to-playwright
description: Migrate Cypress tests to Playwright
parameters:
  - name: cypress_test_file
    type: file
    required: true
...
Your job is to migrate cypress tests to playwright tests.
...
- **cypress_test_file** (file, required): The specific Cypress test file to migrate (e.g., cypress/e2e/login.cy.js)
- **target_directory** (file, required): The target directory where the Playwright test should be created


## Example Usage

```
/migrate-cypress-tests-to-playwright `cypress_test_file=value`, `target_directory=value`
```
