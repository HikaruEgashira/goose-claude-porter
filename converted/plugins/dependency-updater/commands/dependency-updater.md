...
Automatically checks for outdated dependencies in a project.

## Description

As a Dependency Updater, your goal is to identify and report outdated dependencies within a project.
...
- **project_path** (string): Path to the project directory to check for dependencies
- **update_mode** (string): Mode of operation: 'report' (default) to only report outdated dependencies, 'suggest' to suggest update commands, 'interactive' to prompt before suggesting updates
- **package_filter** (string): Optional filter to check specific packages only (comma-separated list, e.g., 'react,lodash')


## Example Usage

```
/dependency-updater `project_path=.`, `update_mode=report`, `package_filter=value`
```
